...
class Evolution:
    ...
    def crossover(self, agent1, agent2):
        ...
        child = Agent()
        newNetwork = self.createModel(self.input_size, self.output_size, self.hiddenLayers, "c")
        ...
        for i in self.createWeightLayerList():
            ...
            for j in range(len(layer)):
                ...
                for k in range(len(neuronConnectionGroup)):
                    ...
                    rand = random.random()
                    if(rand<0.3):
                        new_weights[i][j][k] = neuronConnectionGroup[k] + random.random()*5
                    elif(rand>0.7):
                        new_weights[i][j][k] = neuronConnectionGroup[k] - random.random()*5
        agent1.network.set_weights(new_weights)
    def selectBest(self):
